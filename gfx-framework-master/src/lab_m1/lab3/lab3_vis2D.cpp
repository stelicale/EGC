...
void Lab3_Vis2D::Update(float deltaTimeSeconds)
{
    ...
    if (fpsTimer >= 0.5f) {
        averageFPS = (int)(frameCount / fpsTimer);
        cout << "Lab 3 - FPS: " + std::to_string(averageFPS);
        fpsTimer = 0.0f;
        frameCount = 0;
    }*/
...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    // Zoom centered at current logic window center
    float cx = logicSpace.x + logicSpace.width * 0.5f;
    float cy = logicSpace.y + logicSpace.height * 0.5f;
    float dz = 2.0f * deltaTime; // zoom step

    // Q = Zoom Out (increase logic window -> smaller world)
    if (window->KeyHold(GLFW_KEY_Q)) {
        logicSpace.width += dz;
        logicSpace.height += dz;
    }
    // E = Zoom In (decrease logic window -> larger world)
    if (window->KeyHold(GLFW_KEY_E)) {
        logicSpace.width -= dz;
        logicSpace.height -= dz;
        // Clamp to avoid non-positive sizes
        if (logicSpace.width < 0.2f)  logicSpace.width = 0.2f;
        if (logicSpace.height < 0.2f) logicSpace.height = 0.2f;
    }

    // Keep center fixed after resizing
    logicSpace.x = cx - logicSpace.width * 0.5f;
    logicSpace.y = cy - logicSpace.height * 0.5f;
}
